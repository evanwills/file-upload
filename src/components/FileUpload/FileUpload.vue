<script lang="ts">
import { fileData, fileUploadState } from './FileUpload.d';
import { defaultTypes, humanImplode, getAllowedTypes, humanFileSizeToBytes } from './FileUpload.utils';
...
export default {
  ...
  methods: {
    ...
    getFiles: function (e : Event) : void {
      ...
      if (typeof files !== null) {
        for (let a = 0; a < (files as FileList).length; a += 1) {
          this.processFile((files as FileList)[a]);
        }
      }
    },
    ...
    processFile: function (file: File) : void {
      const tmp : fileData = {
        id: this.getUID(),
      ...
      if (tmp.badType === false) {
        if (this.isImage(tmp.type)) {
          ...
          reduce.toBlob(file, { max: 1500 })
            .then((blob : Blob) => {
              ...
              tmp.file = newFile;
              tmp.src = URL.createObjectURL(newFile);
              tmp.ready = true;
              tmp.size = newFile.size;
              tmp.tooBig = newFile.size > this.singleMax;
    ...
    moveLeft: function (e : Event) : void {
      const btn = (e.target as HTMLButtonElement);
      console.log('event:', e);
      console.log('event.target:', e.target);
      console.log('btn:', btn);
    ...
    moveRight: function (e : Event) : void {
      const btn = (e.target as HTMLButtonElement);
      console.log('event:', e);
      console.log('event.target:', e.target);
      console.log('btn:', btn);
    ...
    <article :class="dialogueClass()">
      ...
      <main v-if="uploadList.length > 0">
        <ul :class="carouselClass()">
          <li v-for="file in uploadList" :key="`file-${file.name}`" class="file-upload__carousel__item">
